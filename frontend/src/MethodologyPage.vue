...
对方 Pick C → 我方 Pick B → 对方 Pick A</pre>
        ...
        <section id="lineup-value">
          ...
          <pre>uncertainty = 12 个模拟终局收益的标准差</pre>
...
└─ 已完成 5v5    → 对当前两套阵容直接评分一次</pre>
          ...
          <p>
            直接评分评价的是页面上已经确定的唯一一套 5v5；Pick 推荐评价的是“现在选择某英雄后，12 种可能后续 BP 的平均结果”。因此推荐值会同时受到未来路径和不确定性的影响，即使最终某一套阵容的直接评分很高，也不表示在更早的 BP 阶段一定容易走到这套阵容。
          </p>
