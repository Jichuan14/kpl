...
@media (max-width:520px) { .method-page { width:calc(100% - .8rem); }.method-hero { border-radius:.8rem; }.hero-copy h1 { font-size:3.5rem; }.hero-intro { font-size:.8rem; }.hero-actions { display:grid; }.hero-actions a { justify-content:center; }.terminal-body p { align-items:start; }.truth-strip { grid-template-columns:1fr; }.truth-strip article { min-height:auto; border-right:0; border-bottom:1px solid var(--line); }.truth-strip article:nth-child(3) { border-bottom:1px solid var(--line); }.section-heading h2 { font-size:2.75rem; }.system-map, .signal-grid { grid-template-columns:1fr; }.system-card:last-child { grid-column:auto; }.plain-callout, .honesty-note { grid-template-columns:1fr; }.matchup-row { grid-template-columns:1fr; }.plus { display:none; }.same-board b { text-align:center; }.decision-record dl div { grid-template-columns:1fr; gap:.2rem; }.decision-record dd { text-align:left; }.coach-flow p { margin-top:.8rem; }.closing-link { grid-template-columns:1fr auto; } }
