    ...
    <template v-if="payload && !loading">
      <section class="method-strip">
        ...
        <div>
          ...
          <strong data-i18n-ignore>
            {{ payload.methodology.decay_half_life_days }}{{ language === "zh-CN" ? " 天" : " days" }}
          </strong>
        </div>
        <div>
          <span>As of</span>
...
.method-strip > div { margin:0; padding:1rem 1.15rem; border-right:1px solid var(--line); }.method-strip > div:last-child{border-right:0}.method-strip span,.method-strip strong{display:block}.method-strip span{color:var(--ink-soft);font-size:.6rem;text-transform:uppercase}.method-strip strong{margin-top:.25rem;font:700 1rem var(--display)}
...
@media(max-width:680px){.rankings-page{width:min(100% - 1rem,640px);padding-top:.6rem}.rankings-hero{padding:1.5rem}.method-strip{grid-template-columns:1fr}.method-strip>div{border-right:0;border-bottom:1px solid var(--line)}.method-strip>div:last-child{border-bottom:0}.board-switch{grid-template-columns:1fr}.podium{grid-template-columns:1fr}.podium article{min-height:0}.hero-list.open{grid-template-columns:1fr}.hero-banner{align-items:flex-start}.hero-portrait{width:66px;height:66px}.hero-filters{grid-template-columns:1fr}.player-row{grid-template-columns:25px minmax(130px,1fr) 70px 75px}.player-stat:nth-of-type(n+5){display:none}.player-stat.confidence{display:none}.section-heading,.player-board-head{align-items:flex-start;flex-direction:column;gap:.5rem}.section-heading>p{text-align:left}}
</style>
